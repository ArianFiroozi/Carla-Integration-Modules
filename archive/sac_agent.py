...
class SACAgent:
    def __init__(self, device=None):
        ...
        # Optimizers
        # Separate mean and log_std parameters
        mean_params = []
        logstd_params = []
        for name, param in self.actor.named_parameters():
            if 'log_std' in name:
                logstd_params.append(param)
            else:
                mean_params.append(param)

        self.actor_opt = optim.Adam([
            {'params': mean_params, 'lr': cfg.ACTOR_LR},
            {'params': logstd_params, 'lr': 0.0},
        ], weight_decay=cfg.WEIGHT_DECAY)

        self.logstd_frozen = True
        print(f"Actor mean params: {len(mean_params)}, log_std params: {len(logstd_params)} (FROZEN)")
        ...
        if self.auto_entropy:
            ...
            self.alpha_opt = optim.Adam([self.log_alpha], lr=0.0)  # frozen initially
    ...
    def maybe_unfreeze_exploration(self):
        if self.logstd_frozen and self.train_step >= cfg.LOGSTD_FREEZE_STEPS:
            for param_group in self.actor_opt.param_groups:
                if param_group['lr'] == 0.0:
                    param_group['lr'] = cfg.ACTOR_LR
            self.logstd_frozen = False
            print(f"[Step {self.train_step}] log_std UNFROZEN (lr={cfg.ACTOR_LR})")
        
        if self.alpha_frozen and self.train_step >= cfg.ALPHA_FREEZE_STEPS:
            for param_group in self.alpha_opt.param_groups:
                param_group['lr'] = cfg.ALPHA_LR
            self.alpha_frozen = False
            print(f"[Step {self.train_step}] alpha UNFROZEN (lr={cfg.ALPHA_LR})")
    ...
    def update(self, replay_buffer):
        ...
        grad_norms = {}
        self.maybe_unfreeze_exploration()
        # ---------------------- Critic update ----------------------
        with torch.no_grad():
