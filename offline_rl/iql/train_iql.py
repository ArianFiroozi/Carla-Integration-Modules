...
def populate_buffer(buffer, wrapper, data_dir):
    ...
    print(f"\n[INFO] Found {len(data_files)} episode files in {data_dir}", flush=True)
    total_transitions = 0
    
    for f in tqdm(data_files, desc="Loading Offline Data", unit="ep"):
        d = np.load(f)
    ...
    print(f"\n[INFO] Successfully loaded {total_transitions} transitions into Replay Buffer.\n", flush=True)

# -------------------------------------------------------------
# Logging & Directories Helpers
# -------------------------------------------------------------
def make_experiment_dir():
    run_stamp = datetime.datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
    exp_dir = Path(cfg.REPO_ROOT) / "experiments" / "offline_rl" / f"{run_stamp}_iql"
...
def main():
    ...
    # 1. Setup Logging Directories (Same as AWAC)
    ...
    print(f"[INFO] Starting IQL Offline Training for {train_steps} steps...")
    ...
    for step in range(1, train_steps + 1):
        ...
        if step % 100 == 0:
            ...
            writer.add_scalar("train/mean_q", losses.get("mean_q", 0), step)
            writer.add_scalar("train/mean_v", losses.get("mean_v", 0), step)
        ...
        if step % 1000 == 0:
            ...
            print(f"Step {step:06d}/{train_steps} | "
                  ...
                  f"Q: {losses.get('mean_q', 0):.2f} | "
                  f"V: {losses.get('mean_v', 0):.2f} | "
                  f"Time: {elapsed:.1f}s")

        # Save Model Checkpoint (Changed to 5000 steps)
        if step % 5000 == 0:
            save_path = model_dir / f"iql_model_step_{step}.pt"
    ...
    print(f"[INFO] 🏆 IQL Model successfully saved to {final_path}")
